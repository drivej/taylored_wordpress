...
import { IWooParams, IWooProduct, IWooVariation } from './IWoo';
...
export async function fetchWooProducts(params: Partial<IWooParams> = {}) {
  return fetchWooAPI<IWooProduct[]>(`/products`, { _fields: 'id,sku,name', ...params }, 'get');
}

export async function fetchWooProductBySku(sku: string, params: { [key: string]: string | number } = {}) {
  const p = await fetchWooAPI<IWooProduct[]>(`products`, { sku, ...params });
  return p.data?.[0];
}

export async function fetchWooProduct<T = IWooVariation | IWooProduct>(id: string | number, params: { [key: string]: string | number } = null) {
...
export async function fetchWooProductVariations(id: string | number, params: { [key: string]: string | number } = null) {
  return await fetchWooAPI<IWooProduct[]>(`products/${id}/variations`, params);
}
