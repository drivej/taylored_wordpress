...
function upsert_brands($brands)
{
    ...
    $brand_parent    = wp_insert_term('Brands', 'product_cat', ['slug' => wc_attribute_taxonomy_slug('brands')]);
    $brand_parent_id = 0;
    ...
    $terms               = get_terms(['slug' => $brand_slugs, 'taxonomy' => 'product_cat', 'hide_empty' => false]);
    ...
    foreach ($brand_terms as &$term) {
        if (isset($lookup_term_by_slug[$term['slug']])) {
            ...
            $new_term = wp_insert_term($term['name'], 'product_cat', ['slug' => $term['slug'], 'parent' => $brand_parent_id]);
