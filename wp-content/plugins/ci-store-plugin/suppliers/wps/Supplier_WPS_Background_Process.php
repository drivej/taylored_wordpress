...
include_once WP_PLUGIN_DIR . '/ci-store-plugin/utils/CronJob.php'; // TODO: remove?
include_once WP_PLUGIN_DIR . '/ci-store-plugin/utils/Supplier_Background_Process.php';

class Supplier_WPS_Background_Process extends Supplier_Background_Process
{
    // protected Supplier_t14 $supplier;

    protected function do_task($item)
    {
        $this->supplier->log('do_task() $item=' . json_encode($item));
        ...
        return ['total_pages' => 10, 'page_index' => $page_index];
    ...
    protected function should_continue()
    {
        ...
        if ($this->task_result) {
            ...
            if ($page_index < $total_pages) {
                ...
                $this->supplier->log("should_continue() {$this->action}");
                return false;
            }
        } else {
            $this->supplier->log("should_continue() task_result empty END {$this->action}");
        }
    }
}
