...
trait Supplier_T14_Prices
{
    ...
    {
        $price        = 0;
        $can_purchase = ($item['attributes']['can_purchase'] ?? false) === true;
        ...
        if ($can_purchase) {
            ...
            // $this->log('can_purchase=false found for ' . $item['id']);
            return 0;
        }
    ...
    {
        // attempt to get price from local db
        global $wpdb;
        $table_name = $wpdb->prefix . 't14_price_lookup';
        $sql        = "SELECT price FROM {$table_name} WHERE item_id = %s";
        $query      = $wpdb->prepare($sql, $item_id);
        $result     = $wpdb->get_row($query);
        if ($result && is_numeric($result->price)) {
            return $result->price;
        }
        // get priuce from api
        $data  = $this->get_api("/pricing/{$item_id}");
        $price = $this->extract_price($data);

        // update local db
        $result = $this->insert_prices([['item_id' => $item_id, 'price' => $price]]);
