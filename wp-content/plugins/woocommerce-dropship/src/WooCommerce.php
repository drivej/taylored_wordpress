...
class WooCommerce
{
	...
	{	
		...
		if (!$supplier) return $is_in_stock;
	...
	private function getObjectSupplier($item)
	{
		...
		if ($supplierKey && class_exists($supplierKey)) return new $supplierKey();
		return false;
	}
	...
	public function updateShippedOrders()
	{
		...
		foreach ($supplierOrders as $supplierClass => $orders) {
			if (!class_exists($supplierClass)) continue;
